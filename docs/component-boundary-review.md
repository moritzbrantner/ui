...
| notification-menu      | shell    | keep          | Optimistic read presentation is now explicit and opt-out; callbacks still own notification persistence.                                                                           | None.                                                                                                                                          |
...
| terminal               | labs     | keep-labs     | Terminal remains a labs display primitive with Storybook coverage and token-backed visual classes.                                                                                | Keep in labs; revisit promotion only if cross-app use emerges.                                                                                 |
| timeline               | labs     | promote-later | Simple state-light timeline now has story/test coverage but should stay labs until a tier decision is made.                                                                       | `tier-change`: decide whether it belongs in `patterns` or `stable` after consumer need is confirmed.                                           |
...
- Decision: keep
- Evidence: [src/components/shell/notification-menu.tsx](../src/components/shell/notification-menu.tsx), [src/components/shell/notification-menu.stories.tsx](../src/components/shell/notification-menu.stories.tsx), [src/components/shell/account-and-notification-menu.test.tsx](../src/components/shell/account-and-notification-menu.test.tsx)
- Context check: menu data/callbacks are app-owned; optimistic local read presentation is explicit and opt-out through `optimisticReadState`.
- Follow-up: none.
...
- Decision: keep-labs
- Evidence: [src/components/labs/terminal.tsx](../src/components/labs/terminal.tsx), [src/components/labs/terminal.stories.tsx](../src/components/labs/terminal.stories.tsx), [src/components/labs/terminal.test.tsx](../src/components/labs/terminal.test.tsx), [src/component-registry.ts](../src/component-registry.ts)
- Context check: display primitive is generic and now has Storybook coverage plus token-backed visual classes.
- Follow-up: keep in labs; revisit promotion only if cross-app use emerges.

### labs/timeline

- Decision: promote-later
- Evidence: [src/components/labs/timeline.tsx](../src/components/labs/timeline.tsx), [src/components/labs/timeline.stories.tsx](../src/components/labs/timeline.stories.tsx), [src/components/labs/timeline.test.tsx](../src/components/labs/timeline.test.tsx), [src/component-registry.ts](../src/component-registry.ts)
- Context check: timeline is a simple state-light display primitive with story/test coverage, but it should stay labs until a tier decision is made.
- Follow-up: `tier-change`: decide whether it belongs in `patterns` or `stable` after consumer need is confirmed.
