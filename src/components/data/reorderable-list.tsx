...
type ReorderHandleProps = Omit<React.ComponentProps<"button">, "disabled"> & {
  disabled?: boolean;
};
...
function ReorderableList({ onReorder, className, children, role = "list", ...props }: ReorderableListProps) {
  return (
    <DragDropProvider
      onDragEnd={(event) => {
        if (event.canceled) {
